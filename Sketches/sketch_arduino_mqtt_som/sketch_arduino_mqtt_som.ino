...
IPAddress ip_local (192, 168, 0, 15);         // IP Arduino
IPAddress ip_server(159, 203, 163, 230);   // IP servidor MQTT

// As variáveis a seguir serão utilizadas pelo sensor de som
int soundDetectedPin = 6; 			// Usa Pino 10 como canal de entrada ou saída para sons
int soundDetected = HIGH; 			// Indicativo que algum barulho foi captado
int numReads  = 256; 				// Número de leituras analógicas 
boolean bAlarm = false; 			// Flag indicando a ocorrência de um som
int soundDetectedValue;
long accumulatedValue;
long avgValue;

unsigned long lastSoundDetectTime; // Registra o momento no qual medimos o som captado 

int soundAlarmTime = 500; // Numero de milisegundos enquanto o som é captado 
...
void loop()
{
  ...
  {
    // Detecta presença de som (1), ou não (0). Em caso positivo, vai publicar valor em breve
    soundDetected = digitalRead (soundDetectedPin) ; // read the sound alarm time
    accumulatedValue = 0;
    ...
    if (soundDetected == HIGH) 						// Ouvimos algo
    {
      lastSoundDetectTime = millis(); // registra o instante corrente
